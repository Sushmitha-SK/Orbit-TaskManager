...
const TaskListTable = ({ tableData }) => {
    ...
    const limitedTableData = tableData.slice(0, 5);
            ...
            <table className="min-w-full">
                ...
                <tbody>
                    {limitedTableData.length > 0 ? (
                        limitedTableData.map((task) => {
                            ...
                            return (
                                <tr key={task._id} className='border-t border-gray-200'>
                                    ...
                                    <td className="py-4 px-4 text-[13px] text-nowrap hidden md:table-cell">
                                        {task.createdAt ? moment(task.createdAt).format('D MMM YYYY') : 'N/A'}
                                    </td>
                                    <td className="py-4 px-4 text-[13px] text-nowrap hidden md:table-cell">
                                        {task.dueDate ? moment(task.dueDate).format('D MMM YYYY') : 'N/A'}
                                        {isOverdue(task.dueDate, task.status) && (
                                            <span
                                                className="ml-2 inline-block px-2 py-1 text-xs text-white bg-red-600 rounded">
                                                Overdue
                                            </span>
                                        )}
                                    </td>
                        ...
                        <tr>
                            <td colSpan="5" className="py-4 px-4 text-center text-gray-500 text-[13px]">
                                No recent tasks
                            </td>
                        </tr>
                    )}
