...
const UpdateProjectModal = ({
    ...
    useEffect(() => {
        if (projectDetails) {
            setProjectData({
                ...
                startDate: projectDetails.createdAt
                    ? moment(projectDetails.createdAt).format("YYYY-MM-DD")
                    : "",
                endDate: projectDetails.dueDate
                    ? moment(projectDetails.dueDate).format("YYYY-MM-DD")
                    : "",
    ...
    const isOverdue = () => {
        if (!projectData.endDate) return false;
        const today = moment().startOf("day");
        const endDate = moment(projectData.endDate, "YYYY-MM-DD");
                    ...
                    <div className="p-4 md:p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={({ target }) => {
                                        ...
                                        const currentEndDate = moment(projectData.endDate, "YYYY-MM-DD");

                                        // Validate if the new end date is earlier than the current overdue date
                                        if (currentEndDate.isValid() && moment(newEndDate, "YYYY-MM-DD").isBefore(currentEndDate)) {
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={({ target }) =>
                                    handleValueChange("status", target.value)
                                }
                            >
                        ...
                        <div className="flex justify-end mt-7">
                            ...
                            <button className="add-btn ml-4" onClick={handleSubmit}>
                                Save Changes
                            </button>
                        </div>
                    </div>
